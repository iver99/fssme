...
EXECUTE IMMEDIATE 'UPDATE EMS_ANALYTICS_CATEGORY A SET PROVIDER_NAME = (SELECT NAME FROM EMS_ANALYTICS_CATEGORY B WHERE  B.CATEGORY_ID = A.CATEGORY_ID AND B.TENANT_ID = A.TENANT_ID AND  A.CATEGORY_ID NOT IN (1,2,4))';
EXECUTE IMMEDIATE 'UPDATE EMS_ANALYTICS_CATEGORY SET PROVIDER_NAME = ' || '''' || 'LoganService' ||  '''' || ',PROVIDER_VERSION =' || '''' || '0.1' || '''' || ' WHERE CATEGORY_ID = 1';
EXECUTE IMMEDIATE 'UPDATE EMS_ANALYTICS_CATEGORY SET PROVIDER_NAME = ' || '''' || 'TargetAnalytics' ||  '''' || ',PROVIDER_VERSION =' || '''' || '1.0.5' || '''' || ' WHERE CATEGORY_ID = 2';
EXECUTE IMMEDIATE 'UPDATE EMS_ANALYTICS_CATEGORY SET PROVIDER_NAME = ' || '''' || 'ApmUI' ||  '''' || ',PROVIDER_VERSION =' || '''' || '0.1' || '''' || ' WHERE CATEGORY_ID = 4';
...
Rem upgrade TA OOB Folders & Searches
@./upgrade_058_059_ta_dml.sql 
