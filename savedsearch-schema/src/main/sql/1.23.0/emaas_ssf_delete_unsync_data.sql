...
BEGIN
  ...
  SELECT COUNT(*) INTO v_count FROM user_tab_columns WHERE table_name='EMS_ANALYTICS_ZDT_COMPARATOR';
  IF v_count > 0 THEN
	SELECT LAST_COMPARISON_DATE into v_date FROM (SELECT to_char(LAST_COMPARISON_DATE,'yyyy-mm-dd hh24:mi:ss.ff3') as LAST_COMPARISON_DATE FROM EMS_ANALYTICS_ZDT_SYNC WHERE SYNC_RESULT = 'SUCCESSFUL' ORDER BY SYNC_DATE DESC) WHERE ROWNUM = 1;
	DBMS_OUTPUT.PUT_LINE('Last compare date is ' || v_date);
  
	delete from EMS_ANALYTICS_ZDT_COMPARATOR where comparison_date > to_timestamp(v_date,'yyyy-mm-dd hh24:mi:ss.ff');
