...
CREATE TABLE EM_ANALYTICS_FOLDERS
  (
    ...
    SYSTEM_FOLDER          NUMBER(1,0) default 0 not null constraint EM_ANALYTICS_FLD_SYSF1 check (SYSTEM_FOLDER between 0 and 1) ,
    EM_PLUGIN_ID           VARCHAR2(64),
    UI_HIDDEN              NUMBER(1,0) default 0 not null constraint EM_ANALYTICS_FLD_UIHID1 check (UI_HIDDEN between 0 and 1),
    CONSTRAINT EM_ANALYTICS_FOLDERS_PK PRIMARY KEY (FOLDER_ID) USING INDEX,
    CONSTRAINT EM_ANALYTICS_FOLDERS_U01 UNIQUE (NAME, PARENT_ID) USING INDEX,
    CONSTRAINT EM_ANALYTICS_FOLDERS_FK1 FOREIGN KEY (PARENT_ID) REFERENCES EM_ANALYTICS_FOLDERS (FOLDER_ID) 
  )  ;
...
CREATE TABLE EM_ANALYTICS_CATEGORY
  (
    ...
    CONSTRAINT EM_ANALYTICS_CATEGORY_PK PRIMARY KEY (CATEGORY_ID) USING INDEX,
    CONSTRAINT EM_ANALYICS_CATEGORY_U01 UNIQUE (NAME) USING INDEX,
    CONSTRAINT EM_ANALYTICS_CATEGORY_FK1 FOREIGN KEY (DEFAULT_FOLDER_ID) REFERENCES EM_ANALYTICS_FOLDERS (FOLDER_ID)
   )  ;
...
CREATE TABLE EM_ANALYTICS_CATEGORY_PARAMS
  (
    CATEGORY_ID              NUMBER(*,0) NOT NULL,
    NAME                     VARCHAR2(64) NOT NULL,
    PARAM_VALUE                    VARCHAR2(1024),
    CONSTRAINT EM_ANALYTICS_CAT_PARAMS_PK PRIMARY KEY (CATEGORY_ID,NAME) USING INDEX,
    CONSTRAINT EM_ANALYTICS_CAT_PARAMS_FK1 FOREIGN KEY (CATEGORY_ID) REFERENCES EM_ANALYTICS_CATEGORY (CATEGORY_ID)
   );
...
Rem FOLDER_ID - The Id of the folder in the EM_ANALYTICS_FOLDERS table to which the search belongs
...
CREATE TABLE EM_ANALYTICS_SEARCH
  (
    ...
    DESCRIPTION_NLSID      VARCHAR2(256),
    DESCRIPTION_SUBSYSTEM  VARCHAR2(64),
    SYSTEM_SEARCH          NUMBER(1,0) default 0 not null constraint EM_ANALYTICS_SRCH_SSRCH1 check (SYSTEM_SEARCH between 0 and 1),
    EM_PLUGIN_ID           VARCHAR2(64),
    IS_LOCKED          	   NUMBER(1,0) default 0 not null constraint EM_ANALYTICS_SRCH_LCK1 check (IS_LOCKED between 0 and 1),
    METADATA_CLOB      	   NCLOB,
    SEARCH_DISPLAY_STR     VARCHAR2(2048),
    UI_HIDDEN		   NUMBER(1,0) default 0 not null constraint EM_ANALYTICS_SRCH_UIHID1 check (UI_HIDDEN between 0 and 1),
    CONSTRAINT EM_ANALYTICS_SEARCH_PK PRIMARY KEY (SEARCH_ID) USING INDEX ,
    CONSTRAINT EM_ANALYTICS_SEARCH_U01 UNIQUE (NAME, FOLDER_ID, CATEGORY_ID) USING INDEX ,
    CONSTRAINT EM_ANALYTICS_SEARCH_FK1 FOREIGN KEY (CATEGORY_ID) REFERENCES EM_ANALYTICS_CATEGORY (CATEGORY_ID),
    CONSTRAINT EM_ANALYTICS_SEARCH_FK2 FOREIGN KEY (FOLDER_ID) REFERENCES EM_ANALYTICS_FOLDERS (FOLDER_ID) 
  ) ;
...
CREATE TABLE EM_ANALYTICS_SEARCH_PARAMS
  (
    SEARCH_ID 		   NUMBER(*,0) NOT NULL,
    NAME 		   VARCHAR2(64) NOT NULL,
    ...
    CONSTRAINT EM_ANALYTICS_SEARCH_PROPS_PK PRIMARY KEY (SEARCH_ID,NAME) USING INDEX,
    CONSTRAINT EM_ANALYTICS_SEARCH_PARAMS_FK1 FOREIGN KEY (SEARCH_ID) REFERENCES EM_ANALYTICS_SEARCH (SEARCH_ID) 
   );
...
("OBJECT_ID" NUMBER(38,0), 
 "ACCESSED_BY" VARCHAR2(64 BYTE), 
 "OBJECT_TYPE" NUMBER(38,0), 
 "ACCESS_DATE" DATE
);
CREATE UNIQUE INDEX "EM_ANALYTICS_LAST_ACCESS_PK" ON "EMS_ANALYTICS_LAST_ACCESS" ("OBJECT_ID", "ACCESSED_BY", "OBJECT_TYPE");
ALTER TABLE "EMS_ANALYTICS_LAST_ACCESS" ADD CONSTRAINT "EM_ANALYTICS_LAST_ACCESS_PK" PRIMARY KEY ("OBJECT_ID", "ACCESSED_BY", "OBJECT_TYPE");
ALTER TABLE "EMS_ANALYTICS_LAST_ACCESS" MODIFY ("OBJECT_ID" NOT NULL ENABLE);
ALTER TABLE "EMS_ANALYTICS_LAST_ACCESS" MODIFY ("ACCESSED_BY" NOT NULL ENABLE);
ALTER TABLE "EMS_ANALYTICS_LAST_ACCESS" MODIFY ("OBJECT_TYPE" NOT NULL ENABLE);
