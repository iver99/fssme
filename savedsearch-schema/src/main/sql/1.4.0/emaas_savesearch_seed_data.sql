...
DECLARE
    V_COUNT             NUMBER;
    WIDGET_SEARCH_ID    VARCHAR2(5);
    DASHBOARD_LINK_ID   VARCHAR2(3);
BEGIN
  WIDGET_SEARCH_ID := '3024';
  DASHBOARD_LINK_ID := '8';
  SELECT COUNT(1) INTO V_COUNT FROM EMS_ANALYTICS_SEARCH_PARAMS WHERE TENANT_ID='&TENANT_ID' AND SEARCH_ID=WIDGET_SEARCH_ID  PARAM_TYPE=1 AND NAME ='EMS_DASHBOARD_TILE' AND PARAM_VALUE_STR =DASHBOARD_LINK_ID;
  IF (V_COUNT>0) THEN
    DBMS_OUTPUT.PUT_LINE('EMS_DASHBOARD_TILE is already 8 for SEARCH_ID: 3024, no need to update again');
  ELSE
    INSERT INTO EMS_ANALYTICS_SEARCH_PARAMS (SEARCH_ID,NAME,PARAM_ATTRIBUTES,PARAM_TYPE,PARAM_VALUE_STR,PARAM_VALUE_CLOB,TENANT_ID) VALUES (WIDGET_SEARCH_ID, 'EMS_DASHBOARD_TILE', null, 1, DASHBOARD_LINK_ID, null, '&TENANT_ID');    
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Update EMS_DASHBOARD_TILE for SEARCH_ID: 3024 in Schema object: EMS_ANALYTICS_SEARCH_PARAMS successfully for tenant: &TENANT_ID');
  END IF;

EXCEPTION
WHEN OTHERS THEN
  ROLLBACK;
  DBMS_OUTPUT.PUT_LINE('Failed to update EMS_DASHBOARD_TILE on SEARCH_ID: 3024 due to '||SQLERRM);
  RAISE;
END;
/
...
DECLARE
    ...
    DASHBOARD_LINK_ID   VARCHAR2(3);
BEGIN
  WIDGET_SEARCH_ID := '3026';
  DASHBOARD_LINK_ID := '10';
  SELECT COUNT(1) INTO V_COUNT FROM EMS_ANALYTICS_SEARCH_PARAMS WHERE TENANT_ID='&TENANT_ID' AND SEARCH_ID=WIDGET_SEARCH_ID  PARAM_TYPE=1 AND NAME ='EMS_DASHBOARD_TILE' AND PARAM_VALUE_STR =DASHBOARD_LINK_ID;
  IF (V_COUNT>0) THEN
    DBMS_OUTPUT.PUT_LINE('EMS_DASHBOARD_TILE is already 10 for SEARCH_ID: 3026, no need to update again');
  ELSE
    INSERT INTO EMS_ANALYTICS_SEARCH_PARAMS (SEARCH_ID,NAME,PARAM_ATTRIBUTES,PARAM_TYPE,PARAM_VALUE_STR,PARAM_VALUE_CLOB,TENANT_ID) VALUES (WIDGET_SEARCH_ID, 'EMS_DASHBOARD_TILE', null, 1, DASHBOARD_LINK_ID, null, '&TENANT_ID');
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Update EMS_DASHBOARD_TILE for SEARCH_ID: 3026 in Schema object: EMS_ANALYTICS_SEARCH_PARAMS successfully for tenant: &TENANT_ID');
  END IF;

EXCEPTION
WHEN OTHERS THEN
  ROLLBACK;
  DBMS_OUTPUT.PUT_LINE('Failed to update EMS_DASHBOARD_TILE on SEARCH_ID: 3026 due to '||SQLERRM);
  RAISE;
END;
/
