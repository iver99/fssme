...
bash "create_wls_datasource" do
  code <<-EOH
    echo "\n--------------------Create Data Source-------------------------">> #{node["log_dir"]}/savedSearchDatasource.log
    set -e
    echo "Executing command:" >> #{node["log_dir"]}/savedSearchDatasource.log 
    echo "#{wls_home}/common/bin/wlst.sh #{node["log_dir"]}/wls_datasources_savedSearch.py" >> #{node["log_dir"]}/savedSearchDatasource.log 
    #{wls_home}/common/bin/wlst.sh #{node["log_dir"]}/wls_datasources_savedSearch.py >> #{node["log_dir"]}/savedSearchDatasource.log 
  EOH
end
...
bash "deploy_ear" do
  code <<-EOH
    ...
    echo "----------------------Deploying EAR-----------------------------">> #{node["log_dir"]}/savedSearchDatasource.log
    echo "\n hostname= #{node["hostname"]}, fqdn=  #{node["fqdn"]}" >> #{node["log_dir"]}/savedSearchDatasource.log
    set +e
