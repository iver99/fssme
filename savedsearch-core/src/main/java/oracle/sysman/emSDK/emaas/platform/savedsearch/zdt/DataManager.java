		...
		try {
			if (count <= 0) {
				...
				sql = SyncSavedSearchSqlUtil.concatCategoryInsert(categoryId, name, description, owner, creationDate, nameNlsid,
						nameSubSystem, descriptionNlsid, descriptionSubSystem, emPluginId, defaultFolderId, deleted,
				...
				em.createNativeQuery(sql).setParameter(1, categoryId).setParameter(2, name).setParameter(3, description)
						...
						.setParameter(10, emPluginId).setParameter(11, defaultFolderId).setParameter(12, deleted)
						.setParameter(13, providerName).setParameter(14, providerVersion).setParameter(15, providerDiscovery)
						.setParameter(16, providerAssetroot).setParameter(17, tenantId).setParameter(18, dashboardIneligible)
						.setParameter(19, lastModificationDate).executeUpdate();
			}
			else {
				//execute update action
				...
				em.createNativeQuery(sql).setParameter(1, name).setParameter(2, description).setParameter(3, owner)
						...
						.setParameter(10, defaultFolderId).setParameter(11, deleted).setParameter(12, providerName)
						.setParameter(13, providerVersion).setParameter(14, providerDiscovery)
		...
		try {
			if (count <= 0) {
				...
				em.createNativeQuery(sql).setParameter(1, name).setParameter(2, parentId).setParameter(3, description)
						...
						.setParameter(16, folderId).setParameter(17, tenantId).executeUpdate();
			}
			em.getTransaction().commit();
		}
		catch (Exception e) {
		...
		try {
			if (count <= 0) {
				...
				em.createNativeQuery(sql).setParameter(1, objectId).setParameter(2, accessedBy).setParameter(3, objectType)
						...
						.setParameter(7, lastModificationDate).executeUpdate();
			}
			else {
				//execute update action
		...
		try {
			if (count <= 0) {
				...
				sql = SyncSavedSearchSqlUtil.concatSearchInsert(searchId, searchGuid, name, owner, creationDate,
						...
						descriptionNlsid, descriptionSubsystem, systemSearch, emPluginId, isLocked, metaDataClob,
						searchDisplayStr, uiHidden, deleted, isWidget, tenantId, nameWidgetSource, widgetGroupName,
						widgetScreenshotHref, widgetIcon, widgetKocName, viewModel, widgetTemplate, widgetSupportTimeControl,
						widgetLinkedDashboard, widgetDefaultWidth, widgetDefaultHeight, dashboardIneligible, providerName,
						providerVersion, providerAssetRoot);
