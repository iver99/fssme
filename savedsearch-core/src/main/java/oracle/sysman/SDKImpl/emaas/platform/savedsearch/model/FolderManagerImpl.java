...
public class FolderManagerImpl extends FolderManager
{
    ...
    {    	
        try{        	
        	...
        	throw new EMAnalyticsFwkException("Error while connecting to data source, please check the data source details: ", EMAnalyticsFwkException.ERR_DATA_SOURCE_DETAILS, null);
        	}
            else{
                _logger.error("Error while saving the folder: "+folder.getName(), dmlce);
                throw new EMAnalyticsFwkException("Error while saving the folder: "+folder.getName(), EMAnalyticsFwkException.ERR_CREATE_FOLDER, null);
            }
        }
        catch(Exception e){
        	
            _logger.error("Error while saving the folder: "+folder.getName(), e);
            throw new EMAnalyticsFwkException("Error while saving the folder: "+folder.getName(), EMAnalyticsFwkException.ERR_CREATE_FOLDER, null, e);
	...
	public Folder getFolder(long folderId) throws EMAnalyticsFwkException {
        ...
        try{
            ...
            throw new EMAnalyticsFwkException("Folder with the Id "+folderId + " " +"does not exist", EMAnalyticsFwkException.ERR_GET_FOLDER_FOR_ID, null, e);
        	}
        	}
		...
		{	emf =PersistenceManager.getInstance().getEntityManagerFactory();
			...
			for(Folder folder : folders)
			{
				...
				{	
					...
					{
						long id =folder.getParentId()==null ? 0 : folder.getParentId().longValue();
	...
	public Folder updateFolder(Folder folder) throws EMAnalyticsFwkException {
        ...
        try{
        	...
        	throw new EMAnalyticsFwkException("Error while connecting to data source, please check the data source details: ", EMAnalyticsFwkException.ERR_DATA_SOURCE_DETAILS, null);
                ...
                _logger.error("Error while saving the folder: "+folder.getName(), dmlce);
                throw new EMAnalyticsFwkException("Error while saving the folder: "+folder.getName(), EMAnalyticsFwkException.ERR_CREATE_FOLDER, null, dmlce);
            }
        }
        catch(Exception e){
	...
	@Override
	public String[] getPathForFolderId(long folderId)
        ...
        try{
            ...
            throw new EMAnalyticsFwkException("Error occurred while getting path for the folder", EMAnalyticsFwkException.ERR_DELETE_FOLDER, null, e);
        }finally
    	{
    		if(em!=null)
    			em.close();    			
	...
	public List<Folder> getSubFolders(long folderId)
        ...
        try{
        	...
        	if( folderId <=0 )
        		folderList = (List<EmAnalyticsFolder>)em.createNamedQuery("Folder.getRootFolders").getResultList();
            ...
            throw new EMAnalyticsFwkException("Error while retrieving the list of sub-folders for the parent folder: "+folderId, EMAnalyticsFwkException.ERR_GENERIC, null, e);
        	}
        }
	}
	...
	public void deleteFolder(long folderId) throws EMAnalyticsFwkException {
        ...
        try{
            ...
            throw new EMAnalyticsFwkException("Error occurred while deleting the folder", EMAnalyticsFwkException.ERR_DELETE_FOLDER, null, e);
        	}
        	}finally
    	{
    		if(em!=null)
	...
	private Folder createFolderObject(EmAnalyticsFolder folderObj, Folder folder) throws EMAnalyticsFwkException
    {
        ...
        try{
            ...
            throw new EMAnalyticsFwkException("Error while getting the folder object", EMAnalyticsFwkException.ERR_GET_FOLDER, null, e);
        }
        return rtnObj;
    }
...
//        	/EntityManagerFactory emf =PersistenceManager.getInstance().getEntityManagerFactory();
        ...
        }catch(NoResultException nre)
        {
          return null;	
            ...
            throw new EMAnalyticsFwkException("Error occurred while  reading the folder", EMAnalyticsFwkException.ERR_GENERIC, null, e);
        }
		return folderObj;    	
    }
