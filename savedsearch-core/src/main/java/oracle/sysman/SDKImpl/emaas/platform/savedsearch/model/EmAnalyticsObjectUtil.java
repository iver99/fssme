...
class EmAnalyticsObjectUtil
{
	...
	public static EmAnalyticsSearch getEmAnalyticsSearchForAdd(Search search, EntityManager em) throws EMAnalyticsFwkException
	{
		...
		searchEntity.setFederationSupported(search.getFederationSupported() ? 1 : 0);
		searchEntity.setGreenfieldSupported(search.getGreenfieldSupported() ? 1 : 0);
    ...
    @SuppressWarnings("unchecked")
    public static List<EmAnalyticsSearch> findEmSearchListByParamWithoutOwner(String name, String value, EntityManager em) {
        List<EmAnalyticsSearch> searchList = new ArrayList<EmAnalyticsSearch>();
        try {
            searchList = (List<EmAnalyticsSearch>) em.createNamedQuery("SearchParam.getSearchListByParam")
                    .setParameter("name", name).setParameter("value", value).getResultList();
        } catch (Exception ex) {
            // do nothing
        }
        return searchList;
