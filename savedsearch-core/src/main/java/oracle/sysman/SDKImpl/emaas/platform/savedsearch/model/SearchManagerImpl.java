...
public class SearchManagerImpl extends SearchManager
{
	...
	public Search editSearch(Search search) throws EMAnalyticsFwkException
	{
		...
		try {
			...
			if (dmlce.getCause().getMessage().contains("EM_ANALYTICS_SEARCH_U01")) {
			...
			else if (dmlce.getCause().getMessage().contains("EM_ANALYTICS_SEARCH_FK2")) {
	...
	public Search getSearch(long searchId) throws EMAnalyticsFwkException
	{
		...
		try {
			EntityManagerFactory emf = PersistenceManager.getInstance().getEntityManagerFactory();
			em = emf.createEntityManager();
			EmAnalyticsSearch searchObj = em.getReference(EmAnalyticsSearch.class, new Long(searchId));
			if (searchObj != null) {
	...
	public Search getSearchByName(String name, long folderId) throws EMAnalyticsFwkException
	{
		...
		try {
			...
			emf = PersistenceManager.getInstance().getEntityManagerFactory();
			em = emf.createEntityManager();
			EmAnalyticsFolder folder = em.find(EmAnalyticsFolder.class, new Long(folderId));
	...
	public List<Search> getSearchListByCategoryId(long categoryId) throws EMAnalyticsFwkException
	{
		...
		try {
			...
			emf = PersistenceManager.getInstance().getEntityManagerFactory();
			em = emf.createEntityManager();
			EmAnalyticsCategory category = em.find(EmAnalyticsCategory.class, categoryId);
			...
			throws EMAnalyticsFwkException
	{
		...
		try {
			...
			emf = PersistenceManager.getInstance().getEntityManagerFactory();
			em = emf.createEntityManager();
			EmAnalyticsFolder folder = em.find(EmAnalyticsFolder.class, folderId);
					...
					if (search.getId() != null && search.getId() > 0) {
						...
						try {
							if (obj != null && obj instanceof Integer) {
								...
								EmAnalyticsFolder folderObj = em.find(EmAnalyticsFolder.class, id);
						...
						if (searchEntity == null) {
							...
							if (cateObj instanceof Integer) {
								EmAnalyticsCategory categoryObj = em.find(EmAnalyticsCategory.class,
							...
							if (search != null) {
								if (search.getFolderId() != null) {
									try {
										EmAnalyticsFolder fObj = em.find(EmAnalyticsFolder.class, search.getFolderId()
												.longValue());
										searchEntity = (EmAnalyticsSearch) em.createNamedQuery("Search.getSearchByName")
	...
	private EmAnalyticsCategory getEmAnalyticsCategoryBySearch(ImportSearchImpl search, EntityManager em)
	{
		...
		try {
			if (search.getCategoryId() != null) {
				category = em.find(EmAnalyticsCategory.class, new Long(search.getCategoryId()));
			}
			else {
	...
	private EmAnalyticsFolder getEmAnalyticsFolderBySearch(ImportSearchImpl search, EntityManager em)
	{
		...
		try {
			if (search.getFolderId() != null) {
				folder = em.find(EmAnalyticsFolder.class, new Long(search.getFolderId()));
			}
			else {
	...
	{
		// incidentally the field names for EmAnalyticsSearch and SearchImpl is same , hence order by will work here
	...
	/*private EmAnalyticsFolder getEmAnalyticsFolderByCategory(ImportCategoryImpl category , EntityManager em) 
	{
		EmAnalyticsFolder folder =null;
		try
		{
			if(category.getDefaultFolderId()!=null)
			  folder =em.find(EmAnalyticsFolder.class , new Long(category.getDefaultFolderId()));
			else
			{
				if(category.getFolderDetails()!=null){
					...
					try
					{ 
					 folder =  EmAnalyticsObjectUtil.getEmAnalyticsFolderForAdd( (Folder)category.getFolderDetails(),em);
				     folder =(EmAnalyticsFolder) 
