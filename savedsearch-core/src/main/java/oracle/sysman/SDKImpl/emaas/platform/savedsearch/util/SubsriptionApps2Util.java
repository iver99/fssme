...
import oracle.sysman.emSDK.emaas.platform.savedsearch.model.subscription2.AppsInfo;
...
/**
 * Created by chehao on 2017/4/1 13:16.
 */
public class SubsriptionApps2Util {
    private static final Logger LOGGER = LogManager.getLogger(SubsriptionApps2Util.class);
    ...
    public static List<String> getSubscribedAppsList(TenantSubscriptionInfo tenantSubscriptionInfo) {
        ...
        for (SubscriptionApps subscriptionApps : subscriptionAppsList) {
			...
			if (OMCSE_SERVICE_TYPE.equals(subscriptionApps.getServiceType())) {
				LOGGER.info("Service type is {}", OMCSE_SERVICE_TYPE);
        ...
        LOGGER.info("After Mapping AppsInfo is {}",appsInfoList);
        LOGGER.info("After Mapping app list is {}",appList);
