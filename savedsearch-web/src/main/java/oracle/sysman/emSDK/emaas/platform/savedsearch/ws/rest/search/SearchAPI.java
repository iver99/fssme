...
public class SearchAPI
{
	...
	public Response getSearch(@PathParam("id") long searchid)
	{
		...
		try {
			...
			message = JSONUtil.ObjectToJSONString(searchObj);
	...
	private Search createSearchObjectForAdd(JSONObject json) throws EMAnalyticsWSException
	{
		...
		try {
			...
			throw new EMAnalyticsWSException("The categoryId key for search is missing in the input JSON Object",
		...
		try {
			...
			throw new EMAnalyticsWSException("The folderId key for search is missing in the input JSON Object",
		...
		if (json.has("parameters")) {
			...
			for (int i = 0; i < jsonArr.length(); i++) {
				...
				if (type.equals("STRING") | type.equals("CLOB"))
					...
					throw new EMAnalyticsWSException("Invalid type please specify either STRING or CLOB",
