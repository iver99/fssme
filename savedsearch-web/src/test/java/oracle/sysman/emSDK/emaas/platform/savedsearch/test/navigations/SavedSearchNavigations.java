...
public class SavedSearchNavigations
{
	...
	public static void createinitObject() throws Exception
	{
		...
		Response res1 = RestAssured.given().contentType(ContentType.XML).log().everything().header("Authorization", authToken).header(TestConstant.SSF_HEADER, TestConstant.SSF_HEADER).header(TestConstant.OAM_HEADER, TENANT_ID1)
	...
	public void getEntities_nonexistFolderId()
	{
		try {
			...
			Response res = RestAssured.given().log().everything().header("Authorization", authToken)
					.header(TestConstant.SSF_HEADER, TestConstant.SSF_HEADER).header(TestConstant.OAM_HEADER, TENANT_ID1).when()
					.get("/entities?folderId=3333333333333");
