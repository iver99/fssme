...
public class CategoryCRUD
{
	...
	{
		try {
			System.out.println("This test is to perform the crud operations on a Category only with POST/GET/PUT/DELETE method");
			System.out.println("											");
			System.out.println("POST operation is in-progress to create a category");
			System.out.println("											");

			String jsonString = "{ \"name\":\"My Test Category\", \"defaultFolderId\":2,\"parameters\":[{\"name\":\"sample\",\"type\":STRING,\"value\":\"my_value\"}] }";
			Response res = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
			...
			System.out.println("==POST method for caregory is over and asserted the successful category creation");
			System.out.println("											");
			...
			Response res4 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
			...
			Assert.assertEquals(res4.asString(), "Category name My Test Category already exist");
			System.out.println("	Using GET method to retrieve the above category before edit");
			System.out.println("											");
			Response res1 = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
					.get("/category/" + jp.get("id"));
			...
			System.out.println("											");
			System.out.println("	Using PUT method to update the above category");

			String jsonString1 = "{ \"name\":\"MyNEWCategory\", \"folderId\":2 }";
			Response res2 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
					.everything().body(jsonString1).when().put("/category/" + jp.get("id"));
			System.out.println("											");
			...
			System.out.println("==PUT method for caregory is over and asserted the successful edit");
			System.out.println("											");
			System.out.println("	Using DELETE method to remove the above category & verifying its non existence");

			Response res3 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
			...
			System.out.println("											");
			Response res5 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
			...
			Assert.assertEquals(res5.asString(), "Category object by ID: " + jp.get("id") + " does not exist");
			System.out.println("------------------------------------------");
			System.out.println("											");
		}
		catch (Exception e) {
	...
	public void category_missingMandateinfo()
	{
		try {
			...
			Response res1 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
			...
			System.out.println(res1.asString());
			Assert.assertEquals(res1.asString(), "The name key for category is missing in the input JSON Object");
			System.out.println("											");
			System.out.println("POST operation is in-progress & missing with required field: name (param)");
			System.out.println("											");
			...
			System.out.println(res2.asString());
			Assert.assertEquals(res2.asString(), "The name key for category param is missing in the input JSON Object");
			System.out.println("											");
			System.out.println("------------------------------------------");
			System.out.println("											");
		}
	...
	public void category_check_NONexistency_defaultFolderId()
	{
		try {
			...
			System.out.println("											");
			System.out.println("POST operation is in-progress to create a category with non-available defaultFolderId ");
			System.out.println("											");
			...
			System.out.println("											");
			// System.out.println(res.asString());
			Assert.assertEquals(res.asString(), "Folder with id 2222 does not exist");
			System.out.println("------------------------------------------");
			System.out.println("											");
	...
	public void category_badURLs()
	{
		try {
			...
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when().get("/category");
	...
	public void searchesbyCategory()
	{
		try {
			...
			System.out.println("Name of Category:" + jp.get("name"));
			System.out.println("											");
			System.out.println("Now creation of searches in the above category with POST method");
			System.out.println("											");
			 ...
			 * System.out.println("											");
			 */
			System.out.println("==POST operation is completed for creation of Category and searches using the same category");
			System.out.println("											");
			System.out.println("Searches by category name is in-progress using GET method");
			System.out.println("											");
			Response res3 = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
			...
			System.out.println("											");
			System.out.println("Searches in the category (name:" + jp.get("name") + ") are: " + jp3.get("name"));
			...
			System.out.println("											");
			Response res4 = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
			...
			System.out.println("											");
			System.out.println("Searches in the category (id :" + jp.get("id") + ") are: " + jp4.get("name"));

			System.out.println("==Searches by category id is done");
			System.out.println("											");
			System.out.println("Delete category using DELETE method while the searches are defined in that category");
			Response res5 = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
					.delete("/category/" + jp.get("id"));
			System.out.println("Status code is: " + res5.getStatusCode());
			System.out.println(res5.asString());
			Assert.assertTrue(res5.getStatusCode() == 500);
			Assert.assertEquals(res5.asString(), "Error occurred while deleting the category");

			System.out.println("											");
			System.out.println("Delete searches using DELETE method from the category whose id is :" + jp.get("id"));
			Response res6 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
			...
			System.out
					.println("checking the deletion of the search from the category, and how many searches still exist using the category id :"
							+ jp.get("id") + ".");
			Response res7 = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
					.get("/findsearch/category?id=" + jp.get("id"));
			JsonPath jp7 = res7.jsonPath();
			System.out.println("Status code is: " + res7.getStatusCode());
			Assert.assertTrue(res7.getStatusCode() == 200);
			System.out.println("Searches in the category (id :" + jp.get("id") + ") are: " + jp7.get("name"));
			System.out.println("											");
			System.out
			...
			Response res8 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
					.everything().when().delete("/category/" + jp.get("id"));
			System.out.println("											");
			...
			System.out.println("==Category whose id:" + jp.get("id") + " has got deleted successfully");
			System.out.println("------------------------------------------");
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public void searchesbyCategory_badRequest()
	{
		try {
			...
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
					.get("/findsearch/category?id");

			System.out.println("Status code is: " + res.getStatusCode());
			Assert.assertTrue(res.getStatusCode() == 400);
			System.out.println(res.asString());
			Assert.assertEquals(res.asString(), "please give either category id or name");
			System.out.println("											");
			...
			Assert.assertTrue(res1.getStatusCode() == 400);
			System.out.println(res1.asString());
			Assert.assertEquals(res1.asString(), "please give either category id or name");
			System.out.println("											");
			System.out.println("Case3:This test is to validate the response and status when name is missing");
			System.out.println("											");
			Response res2 = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
					.get("/findsearch/category?name=");
	...
	public void searchesbyCategory_id_notExist()
	{
		try {
			...
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
					.get("/findsearch/category?id=6789");

			System.out.println("Status code is: " + res.getStatusCode());
			Assert.assertTrue(res.getStatusCode() == 404);
			System.out.println(res.asString());
			Assert.assertEquals(res.asString(), "Category object by ID: 6789 does not exist");
			System.out.println("											");
			System.out.println("------------------------------------------");
	...
	public void searchesbyCategory_invalid_nameANDid()
	{
		try {
			...
			Assert.assertTrue(res.getStatusCode() == 404);
			System.out.println(res.asString());
			Assert.assertEquals(res.asString(), "Category object by ID: 9999 does not exist");
			System.out.println("											");
			System.out.println("------------------------------------------");
	...
	public void searchesbyCategory_caseCheck()
	{
		try {
			...
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
					.get("/findsearch/category?name=ita");

			System.out.println("Status code is: " + res.getStatusCode());
			Assert.assertTrue(res.getStatusCode() == 404);
			System.out.println(res.asString());
			Assert.assertEquals(res.asString(), "Category object by Name: ita does not exist");
			System.out.println("											");
			System.out.println("------------------------------------------");
	...
	public void searchesbyCategory_nameANDidCombinations()
	{
		try {
			...
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when()
					.get("/findsearch/category?id&name=LA");
			...
			System.out.println(res1.asString());
			Assert.assertEquals(res1.asString(), "Category object by ID: 4321 does not exist");
			System.out
	...
	/**
	 * This test is to check the ability of fetch,update and deletion by its name instead of ID
	 */
	@Test
	public void category_CRUDOperationsByName()
	{
		try {
			...
			Response res1 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
					.everything().body(jsonString1).when().put("/category?name=" + jp.get("name"));
			System.out.println("											");
			System.out.println("Status code is: " + res1.getStatusCode());
			System.out.println("											");
			System.out.println(res1.asString());
			...
			System.out.println("											");
			Response res2 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
					.everything().when().delete("/category?name=" + jp.get("name"));
			System.out.println("											");
			System.out.println("Status code is: " + res2.getStatusCode());
			System.out.println("											");
			System.out.println(res2.asString());
			Assert.assertTrue(res2.getStatusCode() == 404);
			Assert.assertEquals(res2.asString(), "Category object by Name: " + jp.get("name") + " does not exist");
			System.out.println("											");
			System.out.println("Retrieve the category by its name using GET method");
			Response res3 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
					.everything().when().get("/category?name=" + jp1.get("name"));
			...
			System.out.println("											");
			Response res4 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
					.everything().when().delete("/category?name=" + jp2.get("name"));
			System.out.println("											");
			System.out.println("Status code is: " + res4.getStatusCode());
			//System.out.println("											");
			System.out.println(res4.asString());
