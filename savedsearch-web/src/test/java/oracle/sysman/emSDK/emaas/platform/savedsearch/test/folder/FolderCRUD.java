...
public class FolderCRUD
{
	...
	public void folder_create()
	{
		try {
			...
			Assert.assertEquals(res2.asString(), "Folder with name Custom Folder already exist");
			System.out.println("    ");

			System.out.println("Verifying weather the folder created or not with GET method");
			System.out.println("											");
		...
		try {
			...
			for (int i = 0; i < a.size(); i++) {
				if (a.get(i).equals("Custom Folder")) {
					...
					Response res1 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
							.everything().body(jsonString).when().put("/folder/" + myfolderID);
					System.out.println("											");
					System.out.println("Status code is: " + res1.getStatusCode());
					System.out.println("											");
					System.out.println(res1.asString());
		...
		try {
			...
			for (int i = 0; i < a.size(); i++) {
				if (a.get(i).equals("Custom Folder_Edit")) {
					...
					System.out.println("											");
					Response res1 = given().contentType(ContentType.JSON).headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log()
							.everything().when().delete("/folder/" + myfolderID);
					System.out.println("											");
					System.out.println("Status code is: " + res1.getStatusCode());
					System.out.println("											");
					System.out.println(res1.asString());
	...
	public void folder_check_NONexistency()
	{
		try {
			...
			System.out.println("											");
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when().get("/folder/333");
	...
	public void folder_badURLs()
	{
		try {
			...
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when().get("/folder");
	...
	public void folder_noType()
	{
		try {
			...
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when().get("/1");
	...
	public void folder_InvalidType()
	{
		try {
			...
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when().get("/1?type=me");
	...
	public void fetchfolder_notExist()
	{
		try {
			...
			System.out.println("											");
			System.out.println(res.asString());
			Assert.assertEquals(res.asString(), "Folder with the Id 99999 does not exist");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void systemFolder_Delete()
	{
		try {
			...
			Response res = given().headers("X-USER-IDENTITY-DOMAIN-NAME", HOSTNAME).log().everything().when().delete("/folder/1");
			// JsonPath jp = res.jsonPath();
			System.out.println(res.asString());
			System.out.println("											");
			System.out.println("Status code is: " + res.getStatusCode());
			Assert.assertTrue(res.getStatusCode() == 500);
			Assert.assertEquals(res.asString(), "Folder with id:" + 1 + " is a system folder and cant be deleted");
			System.out.println("											");
